...
######
    ...
    find . -iname "*.mp4" -iname "*.mkv" -or -iname "*.mp4" -or -iname "*.mpeg" -or -iname "*.avi" -or -iname "*.mov" -or -iname "*.mkv" -or -iname "*.m4v" -or -iname "*.wmv"  | sort -u | while read vidfile
    do
...
#            if [ ! -f "$viddir/$vidbasefilename.jpg" ]; then
#                echo "didn't find $viddir/$vidbasefilename.jpg"
#                echo "Creating thumbnail"
                # from http://superuser.com/questions/238073/make-thumbnail-from-video
                # Get the time as h:m:s (non-padded)
#                l=$(ffmpeg -i "$vidfullfn" 2>&1 | grep Duration: | sed -r 's/\..*//;s/.*: //;s/0([0-9])/\1/g')
...
#                s=$((($(cut -f1 -d: <<< "$l") * 60 + $(cut -f2 -d: <<< "$l")) * 60 + $(cut -f3 -d: <<< "$l")))
                # Get frame at 25% as the thumbnail
#                ffmpeg -ss $((s / 3)) -y -i "$vidfullfn" -r 1 -updatefirst 1 -frames 1 "$viddir/$vidbasefilename.jpg"
#            fi
        fi
    done
